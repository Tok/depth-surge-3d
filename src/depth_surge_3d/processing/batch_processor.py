"""
Batch processor for video-optimized processing mode.

This module implements true batch processing using Video-Depth-Anything
...
class BatchProcessor:
    """
    Handles batch video processing with temporal consistency.

    Uses Video-Depth-Anything model to process entire videos with
    temporal consistency, providing better quality than frame-by-frame processing.
    """

    def __init__(self, depth_estimator: Union[DepthEstimator, VideoDepthEstimator], verbose: bool = False):
        self.depth_estimator = depth_estimator
        self.verbose = verbose
        self.use_video_model = isinstance(depth_estimator, VideoDepthEstimator)
    ...
    def process(
        ...
        try:
            ...
            print(f"Mode: batch (temporal consistency)")
            if self.use_video_model:
                print(f"Using Video-Depth-Anything model for improved quality\n")
            else:
                print(f"Warning: Using frame-by-frame model in batch mode\n")
            ...
            # Step 3: Batch depth estimation
            print("Step 3/7: Generating depth maps (batch mode with temporal consistency)...")
            progress_tracker.update_progress(
