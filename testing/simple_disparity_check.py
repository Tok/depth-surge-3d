#!/usr/bin/env python3
"""
Simple disparity verification without OpenCV dependencies
"""

from PIL import Image
import numpy as np
from pathlib import Path
import argparse

def simple_disparity_check(left_path, right_path):
    """Simple disparity check using PIL"""
    left = Image.open(left_path).convert('L')
    right = Image.open(right_path).convert('L')
    
    left_array = np.array(left)
    right_array = np.array(right)
    
    height, width = left_array.shape
    print(f"Image size: {width}x{height}")
    
    # Expected disparity ranges based on our formula
    max_disparity = width * 0.05   # 5% of width
    min_disparity = width * 0.005  # 0.5% of width
    
    print(f"Expected disparity range: {min_disparity:.1f} to {max_disparity:.1f} pixels")
    print()
    
    # Test regions from our test pattern
    regions = {
        'background': (slice(50, 150), slice(50, 150)),           # Background (depth ~0.8)
        'close_circle_1': (slice(550, 650), slice(350, 450)),     # Red circle area (depth ~0.1)
        'close_circle_2': (slice(550, 650), slice(950, 1050)),    # Second red circle (depth ~0.1)
        'medium_rect_1': (slice(250, 350), slice(250, 450)),      # First green rectangle (depth ~0.7)
        'medium_rect_2': (slice(350, 450), slice(750, 950)),      # Second green rectangle (depth ~0.5)
        'medium_rect_3': (slice(450, 550), slice(1250, 1450)),    # Third green rectangle (depth ~0.3)
    }
    
    print("Simple Disparity Analysis:")
    print("="*50)
    
    for region_name, (y_slice, x_slice) in regions.items():
        try:
            # Extract regions
            left_region = left_array[y_slice, x_slice]
            right_region = right_array[y_slice, x_slice]
            
            # Simple horizontal shift detection
            left_mean = np.mean(left_region)
            right_mean = np.mean(right_region)
            
            # Check if regions are similar (should be for successful stereo)
            brightness_diff = abs(left_mean - right_mean)
            
            # Simple edge detection to measure shift
            left_center_row = left_region[left_region.shape[0]//2, :]
            right_center_row = right_region[right_region.shape[0]//2, :]
            
            # Find edges in both rows
            left_edges = np.where(np.abs(np.diff(left_center_row)) > 20)[0]
            right_edges = np.where(np.abs(np.diff(right_center_row)) > 20)[0]
            
            # Estimate expected disparity based on our formula
            if 'background' in region_name:
                expected_depth = 0.8
            elif 'close_circle' in region_name:
                expected_depth = 0.1
            elif 'rect_1' in region_name:
                expected_depth = 0.7
            elif 'rect_2' in region_name:
                expected_depth = 0.5
            elif 'rect_3' in region_name:
                expected_depth = 0.3
            else:
                expected_depth = 0.5
            
            expected_disparity = max_disparity - (expected_depth * (max_disparity - min_disparity))
            
            # Simple shift estimation
            if len(left_edges) > 0 and len(right_edges) > 0:
                # Find closest edge pair
                min_shift = float('inf')
                for left_edge in left_edges:
                    for right_edge in right_edges:
                        shift = abs(left_edge - right_edge)
                        if shift < min_shift:
                            min_shift = shift
                
                print(f"{region_name:15}: ~{min_shift:4.0f}px shift (expected: {expected_disparity:5.1f}px, depth: {expected_depth:.1f})")
            else:
                print(f"{region_name:15}: No clear edges detected (expected: {expected_disparity:5.1f}px, depth: {expected_depth:.1f})")
            
            if brightness_diff > 10:
                print(f"                 WARNING: Large brightness difference ({brightness_diff:.1f})")
                
        except Exception as e:
            print(f"{region_name:15}: ERROR - {str(e)}")
    
    print()
    print("Visual Check:")
    print("- Compare left vs right images manually")
    print("- Red circles should have LARGEST horizontal shift")
    print("- Background should have SMALLEST shift")
    print("- Green rectangles should have shifts between circles and background")

def main():
    parser = argparse.ArgumentParser(description='Simple disparity verification')
    parser.add_argument('output_dir', help='Output directory containing stereo frames')
    parser.add_argument('--frame', type=int, default=1, help='Frame number to analyze')
    
    args = parser.parse_args()
    
    output_dir = Path(args.output_dir)
    
    # Look for final stereo frames
    left_frames_dir = output_dir / "6_left_frames_final"
    right_frames_dir = output_dir / "6_right_frames_final"
    
    if not left_frames_dir.exists() or not right_frames_dir.exists():
        print("Error: Could not find final stereo frame directories")
        print(f"Looking for: {left_frames_dir} and {right_frames_dir}")
        return
    
    # Find frame files
    left_files = sorted(left_frames_dir.glob("*.png"))
    right_files = sorted(right_frames_dir.glob("*.png"))
    
    if not left_files or not right_files:
        print("Error: No frame files found")
        return
    
    if args.frame > len(left_files):
        print(f"Error: Frame {args.frame} not found (available: 1-{len(left_files)})")
        return
    
    # Analyze the specified frame
    frame_idx = args.frame - 1
    left_path = left_files[frame_idx]
    right_path = right_files[frame_idx]
    
    print(f"Verifying disparity in frame {args.frame}:")
    print(f"  Left: {left_path}")
    print(f"  Right: {right_path}")
    print()
    
    simple_disparity_check(left_path, right_path)

if __name__ == "__main__":
    main()